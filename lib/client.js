...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var CSS =
      ...
      '.dsh-vsced-wiz-backdrop{position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:100;display:flex;align-items:center;justify-content:center}' +
      '.dsh-vsced-wiz{background:var(--dsw-alias-bg-layer-2,#2a2a2e);border:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.35));border-radius:12px;width:480px;max-width:92vw;padding:18px 20px;color:#ddd;font-size:13px;box-shadow:0 12px 40px rgba(0,0,0,.4)}' +
      ...
      '.dsh-vsced-wiz-detail{color:#999;font-size:12px;margin-top:2px;word-break:break-all}' +
      '.dsh-vsced-wiz-log{margin-top:10px;font-size:12px;color:#999;line-height:1.7}' +
    ...
    function statusOf(st) {
      ...
      if (st.backend === 'local' || (st.config && st.config.editorBackend === 'local')) {
        if (st.extConnected) return { color: '#3fb950', text: '本机 VS Code 已连接' }
    ...
    function openWizard() {
      ...
      async function run() {
        ...
        // 3. 等待扩展连接（轮询，最长 120s）
        setStep(2, 'running', '请在 VS Code 中打开 DSH 当前工作区；刚装/更新过扩展需 Reload Window')
        var deadline = Date.now() + 120000
        while (Date.now() < deadline) {
          if (!wizardEl) return // 用户取消
          var st = await fetchState().catch(function () { return null })
          if (st && st.extConnected) {
            phase = 'ok'
            var w = st.extReady && st.extReady.workspace
            setStep(2, 'ok', w ? '已连接窗口：' + w : '已连接')
            return
          }
          if (st && st.workspace) setStep(2, 'running', '等待扩展连接… 请确认 VS Code 已打开工作区 ' + st.workspace + '（必要时 Reload Window）')
          await new Promise(function (res) { setTimeout(res, 1500) })
        }
        phase = 'fail'
        failInfo = '120 秒内未收到扩展连接'
        setStep(2, 'fail', '扩展未连接：请确认扩展已启用、窗口已 Reload、工作区打开正确')
      }
      render()
      run()
    }
