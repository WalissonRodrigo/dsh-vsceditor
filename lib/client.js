...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function statusOf(st) {
      ...
      if (st.install && st.install.phase === 'running') return { color: '#d29922', text: '安装 code-server 中' + (st.install.progress ? ' ' + st.install.progress : '') }
    ...
    function openInstallWizard() {
      ...
      function render() {
        ...
        if (dlDest || dlUrl) {
          ...
          if (dlUrl) html += '下载地址：' + esc(dlUrl) + '<br>'
          if (dlDest) html += '安装到：' + esc(dlDest)
          html += '</div>'
        }
        for (var i = 0; i < steps.length; i++) {
          var s = steps[i]
        ...
        html += '<div class="dsh-vsced-wiz-btns">'
        if (phase === 'running') html += '<button class="dsh-vsced-btn" data-act="cancel">取消安装</button><button class="dsh-vsced-btn" data-act="close">后台继续</button>'
        ...
        while (Date.now() < deadline) {
          ...
          if (!ins || ins.phase === 'running') {
            ...
            setStep(0, 'running', cancelRequested
              ? '正在取消…'
              : (prog ? prog + ' · ' : '') + (last || '下载中（网络慢时可能需要几分钟）…'))
            ...
            if (st.running) {
              ...
              // 第三步打勾。
              phase = 'ok'
              okNote = '安装完成！点「完成」关闭弹窗，然后点击对话界面上方的「编辑器」标签页即可开始使用。'
              setStep(2, 'running', '差最后一步：关闭本弹窗，点击对话界面上方的「编辑器」标签页；页面加载后扩展会自动完成握手')
    ...
    function EditorView() {
      ...
      if (isLocal) {
        ...
        children.push(React.createElement('div', { className: 'dsh-vsced-empty', key: 'em' },
          ...
          }, installing ? '⏳ 正在安装 ' + (st.install.progress || '') + '…（查看进度）' : '⬇ 一键安装 code-server') : null,
          ...
          cfg.editorBackend === 'local' ? null : (st && (st.install && st.install.phase === 'running' || st.lastError && st.lastError.indexOf('未找到 code-server') >= 0))
            ? React.createElement('div', { className: 'row' },
              React.createElement('button', {
                className: 'dsh-vsced-btn',
                onClick: function () { openInstallWizard() },
              }, st.install && st.install.phase === 'running' ? '⏳ 正在安装 ' + (st.install.progress || '') + '…（查看进度）' : '⬇ 一键安装 code-server'),
