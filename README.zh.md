...
- **设置页集成** —— 「设置 → 插件 → 插件配置」里有本插件的折叠卡片：跟随开关、自动启动、端口、code-server 目录、界面语言，全部即时生效并持久化（`~/.dsh/settings.yaml`）
- **多语言支持 (i18n)** —— 内置简体中文 (`zh`)、English (`en`)、Português-BR (`pt-BR`) 与 Español (`es`)，支持跟随浏览器自动检测
...
code-server [不提供官方 Windows 构建](https://github.com/coder/code-server/issues/1397)，且在 Windows 上直接 `npm install -g code-server` 会失败（postinstall 脚本与 argon2 原生编译均会报错）。本插件提供了 `scripts/install-code-server.ps1`，借鉴 [naspenang/code-server-windows](https://github.com/naspenang/code-server-windows) (MIT) 的思路：跳过 postinstall，手动安装依赖，并**借用已安装桌面版 VS Code 的原生模块**。

前置条件：

- Windows 10/11 + PowerShell
- 已安装**桌面版 VS Code**，版本需与 code-server 内置版本**严格一致**（脚本会自动检测并提示版本；可用 `-CodeServerVersion` 指定匹配版本，或 `-SkipVSCodeVersionCheck` 强制跳过）
...
注：此路径未经广泛验证，仅保证 127.0.0.1 本地使用。遇到问题建议使用 **WSL2 运行官方维护的 Linux 流程**，体验与 macOS/Linux 一致。
...
顶栏出现「编辑器」标签页，点击后等待数秒 code-server 就绪。标签页旁的状态指示点：灰色 = 加载中，绿色 = 扩展已连接，黄色 = 等待扩展连接，红色 = 未运行 / 未安装 code-server / 桥接未挂载。

## 5. 使用

### 5.1 本机 VS Code 模式

在「设置 → 插件 → 插件配置」中将**编辑器后端**切换为「本机 VS Code」（与内嵌 code-server 互斥，即时生效），或点击编辑器标签页状态卡片上的**连接向导**按钮：

1. 插件自动探测本机 VS Code（macOS `.app` 与 Spotlight，Windows 常见安装目录与 `where`，Linux `/usr/bin` 与 `which`）；未探测到可在设置中手动填入路径
2. 未安装桥扩展时，状态卡片提供「安装扩展到本机 VS Code」按钮 —— 一键拷贝到 `~/.vscode/extensions/`（用户目录，无需管理员权限）；安装失败会打印手动拷贝路径
3. 在桌面 VS Code 中 Reload Window 并**打开与 DSH 会话相同的工作区** —— 扩展只响应工作区匹配的窗口，多窗口互不干扰
4. 此后跟随 diff 与文件锁定体验与内嵌模式完全一致；扩展随插件版本自动更新（提示时 Reload Window 即可）

原理：环境变量无法注入已运行的桌面应用，host 将桥接参数（端口/token/工作区）写入 `~/.dsh-editor/bridge.json`，扩展轮询该文件完成握手。一套扩展代码自适应双模式，不影响内嵌模式。

#### 工作区信任（Workspace Trust）

桌面 VS Code 对新打开的目录默认启用[受限模式](https://code.visualstudio.com/docs/editor/workspace-trust)。插件对其做了完整适配：

- 桥扩展声明了 `untrustedWorkspaces: limited` 支持 —— **在未信任窗口中仍能激活并维持握手**，但不执行编辑/跳转指令
...
| `language` | string | `auto` | 界面语言：`auto` = 跟随浏览器/DSH 界面语言；也可指定 `zh`、`en`、`pt-BR`、`es` |
