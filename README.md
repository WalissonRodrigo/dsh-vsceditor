...
- **Settings page integration** — a collapsible card under Settings → Plugins → Plugin Configuration: follow toggle, auto-start, port, code-server home, UI language. Everything applies live and persists (`~/.dsh/settings.yaml`)
- **Multi-language support (i18n)** — built-in support for English (`en`), Simplified Chinese (`zh`), Brazilian Portuguese (`pt-BR`), and Spanish (`es`), with auto-detection based on browser locale
...
└────────────────────────────────────────────────────────────┘
        ▲ iframe (client.js registers a conversation.view
          tab "Editor" ("编辑器"), persistent on body across tab switches)
```
