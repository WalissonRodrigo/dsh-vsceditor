...
const CONFIG_DEFAULTS = { follow: true, followWorkspaceOnly: false, autoStart: true, port: 0, codeServerHome: '', editorBackend: 'embedded', vscodePath: '' }
...
function configSchema(value) {
  ...
  if (value.followWorkspaceOnly !== undefined && typeof value.followWorkspaceOnly !== 'boolean') throw new TypeError('followWorkspaceOnly 必须是布尔值')
...
configSchema.dict = {
  ...
  followWorkspaceOnly: { type: 'boolean', meta: { default: CONFIG_DEFAULTS.followWorkspaceOnly, description: '仅跟随工作区内文件：开启后工作区外的改动只记录到最近列表，不弹 diff' } },
...
const plugin = {
  ...
  apply(ctx, config) {
        ...
        readBody(req, 65536, (body) => {
          try {
            ...
            if (msg && msg.type === 'ready') {
              ...
              if (state.extReady.version === BUNDLED_EXT_VERSION && state.notice.indexOf('Reload Window') !== -1) state.notice = ''
            }
    ...
    // polls progress through state.install (phase/log/error).
    function startInstall() {
      ...
      state.install = { phase: 'running', log: [], startedAt: Date.now(), error: '' }
      let proc
      try {
      ...
      proc.done.then((out) => {
        ...
        if (out.exitCode === 0) {
          ...
          state.lastError = ''
          state.notice = 'code-server 安装完成，正在启动…'
          adoptFromExisting()
          startServer()
        } else {
          let tail = ''
    ...
    function startServer() {
      ...
      if (!found) {
        const localHint = '不装 code-server 也可以改用「本机 VS Code」模式（设置 → 插件配置 → 编辑器后端），跟随/锁定体验一致'
        state.lastError = process.platform === 'win32'
          ? '未找到 code-server（Windows 布局：code-server/node/node.exe + code-server/runtime/…/entry.js；查找过 配置的 codeServerHome、$DSH_VSCEDITOR_HOME、<工作区>/.dsh-editor、~/.dsh-editor）。请运行 scripts/install-code-server.ps1 安装；' + localHint
          : '未找到 code-server（查找过 配置的 codeServerHome、$DSH_VSCEDITOR_HOME、<工作区>/.dsh-editor、~/.dsh-editor）。请运行 scripts/install-code-server.sh 安装；' + localHint
        return
      }
