...
const CONFIG_DEFAULTS = { follow: true, autoStart: true, port: 0, codeServerHome: '', editorBackend: 'embedded', vscodePath: '' }
...
function configSchema(value) {
  ...
  if (value.editorBackend !== undefined && value.editorBackend !== 'embedded' && value.editorBackend !== 'local') throw new TypeError('editorBackend 必须是 embedded 或 local')
  if (value.vscodePath !== undefined && typeof value.vscodePath !== 'string') throw new TypeError('vscodePath 必须是字符串')
...
configSchema.dict = {
  ...
  editorBackend: { type: 'string', meta: { default: CONFIG_DEFAULTS.editorBackend, description: '编辑器后端：embedded = 内嵌 code-server；local = 连接本机桌面版 VS Code' } },
  vscodePath: { type: 'string', meta: { default: CONFIG_DEFAULTS.vscodePath, description: '手动指定本机 VS Code 路径（code CLI 或 .app/Code.exe）；留空自动探测' } },
}
  ...
  apply(ctx, config) {
        ...
        readBody(req, 65536, (body) => {
          try {
            ...
            if (msg && msg.type === 'ready') {
              state.extReady = { mode: msg.mode || 'embedded', version: msg.version || '', workspace: msg.workspace || '', at: Date.now() }
            }
          } catch (e) {}
          res.statusCode = 204
          res.end()
    ...
    disposers.push(webServer.register({
      ...
      handler(req, res) {
        adoptFromExisting()
        if (currentConfig.editorBackend === 'local') {
          detectVSCode(false).then((d) => { state.desktop = d; sendJson(res, snapshot()) })
            .catch(() => sendJson(res, snapshot()))
          return
        }
        ...
        readBody(req, 65536, (body) => {
          try {
            ...
            if (msg.action === 'detect-vscode') {
              detectVSCode(true)
                .then((d) => { state.desktop = d; sendJson(res, { ok: true, desktop: d }) })
                .catch((e) => sendJson(res, { ok: false, error: String(e && e.message ? e.message : e) }))
              return
            }
    ...
    function writeBridgeFile() {
      ...
      try {
        ...
        state.lastError = 'bridge.json 写入失败：' + (e && e.message ? e.message : String(e))
      }
    }
    ...
    // > platform search (which/where/mdfind). Cached briefly; the settings
    // card polls state every 2.5s and we must not spawn on every poll.
    ...
    async function detectVSCode(force) {
      if (!force && detectCache.result !== undefined && Date.now() - detectCache.at < 15000) return detectCache.result
      ...
      if (cli) {
        ...
        // Extension detection: CLI first, directory scan as fallback.
    ...
    // Copy the bundled extension into ~/.vscode/extensions/dsh.dsh-bridge-<v>
    // (home dir, no privilege needed on any platform) and remove older copies.
    function installDesktopExtension() {
      const dest = path.join(EXT_INSTALL_ROOT, 'dsh.dsh-bridge-' + BUNDLED_EXT_VERSION)
      try {
        ...
        for (const d of fs.readdirSync(EXT_INSTALL_ROOT)) {
          if (d.indexOf('dsh.dsh-bridge-') === 0 && d !== 'dsh.dsh-bridge-' + BUNDLED_EXT_VERSION) {
    ...
    async function ensureDesktopExtSynced() {
      ...
      if (d.cli && d.extInstalled && !d.extUpToDate) {
        const r = installDesktopExtension()
        if (r.ok) state.notice = '桌面 VS Code 的 DSH 扩展已更新到 v' + BUNDLED_EXT_VERSION + '，请在 VS Code 里 Reload Window 生效'
      }
    }
    ...
    function startServer() {
      ...
      if (!found) {
        state.lastError = process.platform === 'win32'
          ? '未找到 code-server（Windows 布局：code-server/node/node.exe + code-server/runtime/…/entry.js；查找过 配置的 codeServerHome、$DSH_VSCEDITOR_HOME、<工作区>/.dsh-editor、~/.dsh-editor）。请运行 scripts/install-code-server.ps1'
          : '未找到 code-server（查找过 配置的 codeServerHome、$DSH_VSCEDITOR_HOME、<工作区>/.dsh-editor、~/.dsh-editor）'
        return
      }
